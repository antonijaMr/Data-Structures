...
class MojVector {
  public:
  MojVector() : capacity_{1}, size_{0}, arr_{new T[capacity_]} {}  // done
  ...
  MojVector(unsigned int size):capacity_{size},size_{size},arr_{new T[capacity_]}{}
  ...
  void resize(size_t cap) {
    size_t tmpSize = size_ > cap ? cap : size_;//uzimamo manje
    ...
    size_ = cap;
    capacity_ = cap;
    for (auto i = tmpSize; i < size_; ++i) arr_[i] = T{};
  }
